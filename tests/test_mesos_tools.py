...
def mock_yamlload():
    services = {
        'task_processing.mesos_leader_paasta_fake_cluster_fake_region': {
            'host': 'fake_host',
            'port': 'fake_port',
        },
    }
...
def test_get_mesos_proxy(
    ...
    proxy_addr = mesos_tools.get_mesos_proxy('fake_cluster', 'fake_region')
...
def test_get_mesos_proxy_no_proxy(
    ...
    with raises(KeyError) as exc_info:
        mesos_tools.get_mesos_proxy(cluster, region)
